<template>
    <div class="modal fade" id="educational-stages-model" tabindex="-1"
...
<script setup>
  ...
  setTimeout(async () => {
    let myModalEl = document.getElementById('educational-stages-model')
  ...
  function resetModal() {
    ...
    setTimeout(async () => {
      if (props.type != 'edit') {
        ...
        loading.value = true;
         adminApi.get(`educational-stages/${id.value}`)
            .then((res) => {
              let l = res.data.data;
  ...
  const AddSubmit = () =>  {
      ...
      if (props.type !== 'edit') {
        if (!v$.value.$error) {
          is_disabled.value = false;
          loading.value = true;
          adminApi.post(`educational-stages`, formData)
              .then((res) => {
                Swal.fire({
                  icon: 'success',
        ...
        adminApi.post(`educational-stages/${id.value}`,formData)
            .then((res) => {
              Swal.fire({
                icon: 'success',
